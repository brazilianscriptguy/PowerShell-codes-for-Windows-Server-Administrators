...
13. **Create-ScriptMenuGUI.ps1**: Provides a streamlined, automated GUI menu for categorizing and executing PowerShell scripts. Scripts are organized into sections for `ADAdmin Tools` and `EventLogs Tools`, with each section sorted alphabetically.
...
18. **Disable-Expired-ADUserAccounts.ps1**: Automates the disabling of expired user accounts within an Active Directory environment, enhancing security and compliance by ensuring inactive accounts are promptly deactivated.
...
38. **Remove-EmptyFiles-or-DateRange.ps1**: Streamlines file management by detecting and deleting empty files and allowing removal of files within a specified date range, thereby optimizing storage and enhancing system organization.
...
49. **Synchronize-ADComputerTime.ps1**: Ensures accurate workstation time settings across different time zones, utilizing a GUI for easy adjustments.
...
58. **NEXT COMING SOON**: Stay tuned for more innovative and efficient PowerShell scripts that will continue to enhance system administration efficiency and effectiveness.
