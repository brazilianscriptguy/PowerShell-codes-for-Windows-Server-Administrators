...
# Show messages with a dialog box
...
function Display-ExpiredCertificates {
    ...
    foreach ($cert in $allCertificates) {
        if ($cert -ne $null -and $cert.PSObject.Properties.Match('Thumbprint') -and $cert.Thumbprint) {
            $ListBox.Items.Add($cert.Thumbprint)
        } else {
            Write-Log -Message "Skipped an invalid certificate item." -Level "INFO"
        }
    }
...
function Remove-CertificatesByThumbprint {
    ...
    Show-Message -Message "Certificate removal completed." -Type "Information"
}
...
$form.Controls.Add((New-Object System.Windows.Forms.Button -Property @{
    ...
    Size = New-Object System.Drawing.Size(200, 30)
    Add_Click = {
        $thumbprints = @($listBoxCertificates.Items | ForEach-Object { $_.ToString() })
