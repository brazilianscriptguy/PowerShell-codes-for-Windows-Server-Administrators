...
6. **Inactive-Computer-Accounts.ps1**: This PowerShell script generates a report of inactive computer accounts in an Active Directory domain. It prompts the user to input the domain's Fully Qualified Domain Name (FQDN) and the number of days of inactivity. The script then queries Active Directory for inactive computer accounts and exports their information, including name, DNS hostname, and last logon date, to a CSV file.

7. **Lastlogon-UserAccounts-Report.ps1**: This PowerShell script creates a report of user accounts by their last logon activity in an Active Directory environment. Users provide the domain's FQDN and the number of days since their last logon. The script queries Active Directory to find inactive user accounts and generates a CSV report containing user names, SamAccountNames, and their respective last logon dates. It also displays a Windows Forms GUI for user interaction and report generation.

8. **List-Users-Passwors-Never-Expires.ps1**: This script is an essential tool for security auditing in Active Directory environments. It exports a list of users whose passwords are set to never expire. The data is compiled into an easily accessible CSV file and stored in the 'My Documents' folder.

9. **Member-Servers-Forest-Report.ps1**: This script is a valuable tool for system administrators responsible for managing Active Directory environments. It simplifies the task of identifying and exporting member servers within a specified domain, allowing administrators to gather essential information about these servers, such as their names and roles. With a user-friendly interface, it prompts the user to input the Fully Qualified Domain Name (FQDN) of the domain, and then efficiently queries Active Directory to extract relevant server data. The script exports this information into a CSV file, making it a convenient solution for inventory and documentation purposes.
